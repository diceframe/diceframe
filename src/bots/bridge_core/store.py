"""Platform-neutral session mapping and deduplication store."""
...
class JsonBridgeStore:
    ...
    async def load(self) -> None:
        ...
        async with self._lock:
            ...
            recent_commands = data.get("recent_command_signatures", data.get("recent_commands", {}))
    ...
    async def unbind_group(self, stream_id: str) -> None:
        async with self._lock:
            group = self.groups.pop(str(stream_id), None)
            ...
            if game_key:
                ...
                self.players = {
                    ...
                    if not key.startswith(prefix) or value.get("game_key") != game_key
                }
            self._persist_locked()

    def group(self, stream_id: str) -> dict[str, Any] | None:
        return self.groups.get(str(stream_id))

    async def update_group_roster(self, stream_id: str, roster: list[dict[str, Any]]) -> None:
        async with self._lock:
            group = self.groups.get(str(stream_id))
            if not group:
                return
            group["roster"] = roster
            self._persist_locked()
    ...
    async def update_roster(self, stream_id: str, roster: list[dict[str, Any]]) -> None:
        await self.update_group_roster(stream_id, roster)
        ...
        async with self._lock:
            ...
            for key, mapping in self.players.items():
                if key != self.player_key(stream_id, platform_user_id) and mapping.get("game_key") == game_key and mapping.get("user_id") == user_id:
                    return False
            self.players[self.player_key(stream_id, platform_user_id)] = {
                "game_key": game_key,
                "user_id": user_id,
            }
            self._persist_locked()
