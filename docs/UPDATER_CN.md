...
便携版最多保留两套程序文件。初始压缩包根目录里的 `app/` 和 `python/` 算作第一套；第一次更新后它们作为回滚版本保留。第二次及后续更新成功后，程序只保留 `versions/` 中的当前版本和上一版本，并清理根目录里的旧 `app/`、`python/`。`data/`、`logs/` 和根目录启动器不属于版本副本，不会随旧版本清理。当前版本损坏时，启动器会根据版本指针尝试上一版本。
