...
@pytest.mark.asyncio
async def test_plan_round_checks_captures_multilingual_economy_actions() -> None:
    """GM AI 的经济意图（任何语言）经同一 tool call 捕获并进入证据层。"""
    ...
    class FakeClient:
        async def call_tools(self, _system, user, **kwargs):
            tools = kwargs["tools"][0]["function"]["parameters"]["properties"]
            assert "economy_actions" in tools
            return SimpleNamespace(
                tool_calls=[{
                    "name": "dice_checks",
                    "arguments": {
                        "checks": [],
                        "economy_actions": [
                            {"player": "p1", "type": "purchase", "target": "この剣", "amount": 30},
                            {"player": "p2", "type": "purchase", "target": "I want this ring"},
                        ],
                    },
                }],
                total_tokens=20,
    ...
    client = FakeClient()
    _planned, metadata = await plan_round_checks(instance, make_rule(), client)
    assert [a["player"] for a in metadata["economy_actions"]] == ["p1", "p2"]
    assert metadata["economy_actions"][0]["target"] == "この剣"
    assert metadata["economy_actions"][0]["amount"] == 30
    evidence = [item for item in instance.economy["evidence"] if item["type"] == "ai_action"]
    assert len(evidence) == 2
    assert all(item["authority"] is False for item in evidence)
    assert all(item["source"] == "gm_ai" for item in evidence)


@pytest.mark.asyncio
async def test_malformed_economy_actions_are_tolerated() -> None:
    """畸形 economy_actions 不影响检定规划（overreach 同款容错）。"""
        ...
        async def call_tools(self, _system, user, **kwargs):
            return SimpleNamespace(
                tool_calls=[{
                    "name": "dice_checks",
                    "arguments": {
                        "checks": [{"player": "p1", "attribute": "str", "target": 12}],
                        "economy_actions": [
                            "garbage",
                            {"player": "ghost", "type": "purchase", "target": "x"},
                            {"player": "p1", "type": "mind_control", "target": "x"},
    ...
    planned, metadata = await plan_round_checks(instance, make_rule(), client)
    assert len(planned) == 1
    assert metadata["economy_actions"] == []


@pytest.mark.asyncio
async def test_replanned_round_dedupes_ai_action_evidence() -> None:
    instance = make_instance()
            ...
            return SimpleNamespace(
                tool_calls=[{
                    ...
                    "arguments": {
                        "checks": [],
                        "economy_actions": [
                            {"player": "p1", "type": "purchase", "target": "この剣"},
                        ],
                    },
                }],
                total_tokens=5,
    ...
    await plan_round_checks(instance, make_rule(), client)
    count_after_first = len([
        item for item in instance.economy["evidence"] if item["type"] == "ai_action"
    ])
